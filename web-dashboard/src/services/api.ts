...
export class RestaurantDashboardService {
  static async getDailyStats(googlePlaceId: string): Promise<DailyStats[]> {
    try {
      const response = await api.get(`/restaurants/${googlePlaceId}/dashboard/stats`);
      
      // Mock data for demo since backend endpoints are not implemented yet
  ...
  static async getOriginBreakdown(googlePlaceId: string): Promise<OriginStats[]> {
    try {
      const response = await api.get(`/restaurants/${googlePlaceId}/dashboard/origins`);
      ...
      console.error('Error fetching origin breakdown:', error);
      throw error;
    }
  }
  ...
  static async getPerformanceTrends(googlePlaceId: string): Promise<TrendData[]> {
    try {
      const response = await api.get(`/restaurants/${googlePlaceId}/dashboard/trends`);
